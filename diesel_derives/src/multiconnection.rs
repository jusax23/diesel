...
pub fn derive(item: DeriveInput, is_async: bool) -> TokenStream {
    if let syn::Data::Enum(e) = item.data {
        let helper = MultiHelper::new(is_async);
...
struct MultiHelper {
    ...
    /// `.await` or empty
    await_token: TokenStream,
    /// `async` or empty
    async_token: TokenStream,
}
...
fn generate_connection_impl(
    ...
    let simple_impls = if is_async {
        if cfg!(feature = "async_pool") {
            quote::quote! { impl diesel_async::pooled_connection::PoolableConnection for MultiConnection {} }
        } else {
            quote::quote! {}
        }
    } else {
